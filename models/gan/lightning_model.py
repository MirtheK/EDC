...
from EDC.metrics.auroc import AUROC
from EDC.metrics.evaluator import Evaluator
from EDC.metrics.f1_score import F1Score
...
class Ganomaly(nn.Module):
    """GANomaly Model for 3D Medical Image Anomaly Detection.
    ...
    Args:
        input_size (tuple[int, int, int]): Input image size (depth, height, width)
        batch_size (int): Number of samples in each batch.
            Defaults to ``32``.
        n_features (int): Number of feature channels in CNN layers.
            Defaults to ``64``.
    ...
    def __init__(
        ...
        self.min_scores: torch.Tensor = torch.tensor(float("inf"), dtype=torch.float32)
        self.max_scores: torch.Tensor = torch.tensor(float("-inf"), dtype=torch.float32)

        self.model = GanomalyModel(
            input_size=self.input_size,
            num_input_channels=1,  # 3D medical images typically single channel
            n_features=self.n_features,
            latent_vec_size=self.latent_vec_size,
            extra_layers=self.extra_layers,
            add_final_conv_layer=self.add_final_conv_layer,
    ...
    def _reset_min_max(self) -> None:
        """Reset min_max scores."""
        self.min_scores = torch.tensor(float("inf"), dtype=torch.float32)
        self.max_scores = torch.tensor(float("-inf"), dtype=torch.float32)

    def configure_optimizers(self) -> list[optim.Optimizer]:
        """Configure optimizers for generator and discriminator.

        Returns:
            List of optimizers: [discriminator_optimizer, generator_optimizer]
        """
        optimizer_d = optim.Adam(
            self.model.discriminator.parameters(),
            lr=self.learning_rate,
    ...
    def forward(self, x: torch.Tensor) -> tuple:
        ...
        Returns:
            If training: (padded, fake, latent_i, latent_o)
            If inference: anomaly scores
        """
        return self.model(x)
    ...
    def _normalize(self, scores: torch.Tensor) -> torch.Tensor:
        ...
        """
        return (scores - self.min_scores.to(scores.device)) / (
            self.max_scores.to(scores.device) - self.min_scores.to(scores.device) + 1e-8
        )
