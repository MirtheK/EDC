...
class Encoder(nn.Module):
    ...
    Args:
        input_size (tuple[int, int, int]): Size of input image (height, width, depth)
    ...
    Example:
        ...
        ...     n_features=64
        ... )
        >>> input_tensor = torch.randn(32, 3, 256, 256, 256)
    ...
    def __init__(
        ...
        self.input_layers.add_module(
            ...
            nn.Conv3d(num_input_channels, n_features, kernel_size=4, stride=2, padding=4, bias=False),
        )
        ...
        for layer in range(extra_layers):
            self.extra_layers.add_module(
                ...
                nn.Conv3d(n_features, n_features, kernel_size=3, stride=1, padding=1, bias=False),
            )
            self.extra_layers.add_module(f"extra-layers-{layer}-{n_features}-batchnorm", nn.BatchNorm3d(n_features))
        ...
        while pyramid_dim > 4:
            ...
            self.pyramid_features.add_module(
                ...
                nn.Conv3d(in_features, out_features, kernel_size=4, stride=2, padding=1, bias=False),
            )
            self.pyramid_features.add_module(f"pyramid-{out_features}-batchnorm", nn.BatchNorm3d(out_features))
...
class Decoder(nn.Module):
    ...
    Args:
        input_size (tuple[int, in, ind]): Size of output image (height, width, depth)
    ...
    Example:
        ...
        ...     n_features=64
        ... )
        >>> latent = torch.randn(32, 100, 1, 1, 1)
    ...
    def __init__(
        ...
        self.latent_input.add_module(f"initial-{n_input_features}-batchnorm", nn.BatchNorm3d(n_input_features))
        ...
        while pyramid_dim > 4:
            ...
            self.inverse_pyramid.add_module(f"pyramid-{out_features}-batchnorm", nn.BatchNorm3d(out_features))
        ...
        for layer in range(extra_layers):
            self.extra_layers.add_module(
                ...
                nn.Conv3d(n_input_features, n_input_features, kernel_size=3, stride=1, padding=1, bias=False),
            )
            self.extra_layers.add_module(
                f"extra-layers-{layer}-{n_input_features}-batchnorm",
                nn.BatchNorm3d(n_input_features),
            )
            self.extra_layers.add_module(
...
class Discriminator(nn.Module):
    ...
    Args:
        input_size (tuple[int, int, int): Input image size (height, width, depth)
        num_input_channels (int): Number of input image channels
        n_features (int): Number of feature maps in convolution layers
        extra_layers (int, optional): Number of extra intermediate layers.
            Defaults to ``0``.
...
class Generator(nn.Module):
    ...
    Args:
        input_size (tuple[int, int, int]): Input/output image size (height, width, depth)
...
class GanomalyModel(nn.Module):
    ...
    Example:
        ...
        ...     latent_vec_size=100
        ... )
        >>> input_tensor = torch.randn(32, 1, 256, 256, 256)
        >>> output = model(input_tensor)
    ...
    def forward(
        ...
        scores = torch.mean(torch.pow((latent_i - latent_o), 2), dim=tuple(range(1, latent_i.ndim)))
